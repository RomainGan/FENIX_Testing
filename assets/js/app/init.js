...
function importJSON(){
  ...
  input.onchange = async (e) => {
    ...
    try {
      ...
      if(!Array.isArray(data)) throw new Error('Format invalide — tableau JSON attendu');
      const existingIds = new Set(players.map(p=>p.id));
      ...
      _showImportToast('✓ JSON importé — '+added+' joueur(s) ajouté(s), '+updated+' session(s) fusionnée(s)','var(--green)');
    } catch(err) {
